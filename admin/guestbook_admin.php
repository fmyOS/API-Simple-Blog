...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $a = $_POST['action'] ?? '';
    if ($a === 'approve') {
        $pdo->prepare("UPDATE guestbook SET status='approved' WHERE id=?")->execute([intval($_POST['id'])]);
        $msg = '<div class="msg msg-success">留言已通过审核！</div>';
    } elseif ($a === 'delete') {
        $pdo->prepare("DELETE FROM guestbook WHERE id=?")->execute([intval($_POST['id'])]);
        $msg = '<div class="msg msg-success">留言已删除！</div>';
    } elseif ($a === 'approve_all') {
        $pdo->query("UPDATE guestbook SET status='approved'");
        $msg = '<div class="msg msg-success">所有留言已通过审核！</div>';
    }
}
$filter = $_GET['filter'] ?? 'all';
$sql = "SELECT * FROM guestbook";
if ($filter === 'pending') $sql .= " WHERE status='pending'";
elseif ($filter === 'approved') $sql .= " WHERE status='approved'";
$sql .= " ORDER BY created_at DESC LIMIT 200";
$entries = $pdo->query($sql)->fetchAll();
$cnt_all = $pdo->query("SELECT COUNT(*) FROM guestbook")->fetchColumn();
$cnt_pending = $pdo->query("SELECT COUNT(*) FROM guestbook WHERE status='pending'")->fetchColumn();
$cnt_approved = $pdo->query("SELECT COUNT(*) FROM guestbook WHERE status='approved'")->fetchColumn();
...
<div class="top-bar">
    <div style="display:flex; gap:6px; flex-wrap:wrap;">
        ...
        <a href="?filter=pending" class="btn <?php if($filter==='pending') echo 'btn-primary'; else echo 'btn-secondary'; ?>">待审核 (<?php echo $cnt_pending; ?>)</a>
        <a href="?filter=approved" class="btn <?php if($filter==='approved') echo 'btn-primary'; else echo 'btn-secondary'; ?>">已通过 (<?php echo $cnt_approved; ?>)</a>
    </div>
    <?php if ($cnt_pending > 0): ?>
    <form method="post" style="display:inline;"><input type="hidden" name="action" value="approve_all"><button type="submit" class="btn btn-primary" onclick="return confirm('一键通过所有待审留言？')">一键通过全部</button></form>
    <?php endif; ?>
</div>
<?php if(count($entries)): ?>
<table>
<thead><tr><th>ID</th><th>昵称</th><th>邮箱</th><th>内容</th><th>IP</th><th>状态</th><th>时间</th><th>操作</th></tr></thead>
<tbody>
<?php foreach($entries as $g): ?>
<tr>
    <td><?php echo $g['id']; ?></td>
    <td><?php echo e($g['author']); ?></td>
    <td><?php echo e($g['email'] ?? '-'); ?></td>
    <td style="max-width:280px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><?php echo e($g['content']); ?></td>
    <td style="color:rgba(255,255,255,0.35); font-size:0.78rem;"><?php echo e(substr($g['ip']??'',0,20)); ?></td>
    <td><span class="badge <?php echo $g['status']==='approved'?'badge-approved':'badge-pending'; ?>"><?php echo $g['status']; ?></span></td>
    <td style="color:rgba(255,255,255,0.35); font-size:0.78rem;"><?php echo $g['created_at']; ?></td>
    <td class="actions">
        <?php if ($g['status'] !== 'approved'): ?>
        <form method="post" style="display:inline;"><input type="hidden" name="action" value="approve"><input type="hidden" name="id" value="<?php echo $g['id']; ?>"><button type="submit" class="btn btn-primary btn-sm">通过</button></form>
        <?php endif; ?>
        <form method="post" style="display:inline;" onsubmit="return confirm('确认删除？')"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo $g['id']; ?>"><button type="submit" class="btn btn-danger btn-sm">删除</button></form>
    </td>
</tr>
<?php endforeach; ?>
</tbody>
</table>
<?php else: ?><div class="empty">暂无留言</div><?php endif; ?>
</div>
    </div>
</body>
</html>