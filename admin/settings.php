...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $fields = ['site_name','site_desc','site_keywords','contact_email','ICP_license','police_record'];
    foreach ($fields as $key) {
        $val = trim($_POST[$key] ?? '');
        $pdo->prepare("INSERT INTO settings (setting_key, value) VALUES (?,?) ON DUPLICATE KEY UPDATE value=VALUES(value)")->execute([$key, $val]);
    }
    $msg = '<div class="msg msg-success">设置已保存！</div>';
}
$settings = getSettings();
$db_size = $pdo->query("SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) FROM information_schema.tables WHERE table_schema='blog_db'")->fetchColumn();
$api_key = $pdo->query("SELECT api_key FROM api_keys ORDER BY id DESC LIMIT 1")->fetchColumn();
...
<form method="post">
<div class="form-row">
    <div class="form-group"><label>网站名称</label><input name="site_name" value="<?php echo e($settings['site_name'] ?? ''); ?>"></div>
    <div class="form-group"><label>网站描述 (SEO)</label><input name="site_desc" value="<?php echo e($settings['site_desc'] ?? ''); ?>"></div>
</div>
<div class="form-group"><label>网站关键词 (SEO，逗号分隔)</label><input name="site_keywords" value="<?php echo e($settings['site_keywords'] ?? ''); ?>"></div>
<div class="form-row">
    <div class="form-group"><label>联系邮箱</label><input name="contact_email" value="<?php echo e($settings['contact_email'] ?? ''); ?>"></div>
    <div class="form-group"><label>ICP备案号</label><input name="ICP_license" value="<?php echo e($settings['ICP_license'] ?? ''); ?>"></div>
</div>
<div class="form-group"><label>公安联网备案号</label><input name="police_record" value="<?php echo e($settings['police_record'] ?? ''); ?>"></div>
...
<tr><td>MySQL 版本</td><td><?php echo e($mysql_version); ?></td></tr>
<tr><td>Web 服务器</td><td><?php echo e($_SERVER['SERVER_SOFTWARE'] ?? ''); ?></td></tr>
<tr><td>数据库大小</td><td><?php echo $db_size; ?> MB</td></tr>
</table>
</div>
<div class="section">
<h2>🔑 API 密钥</h2>
<table class="info-table">
<tr><td>当前 API Key</td><td><code style="color:#00f5ff; font-size:0.88rem;"><?php echo e($api_key ?? '未设置'); ?></code></td></tr>
<tr><td>Key 管理</td><td><a href="api_keys_admin.php" class="btn btn-secondary btn-sm">前往管理 →</a></td></tr>
</table>
</div>
    </div>
</body>
